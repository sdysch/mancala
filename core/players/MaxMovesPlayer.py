...
class MaxMovesPlayer(Player):
    """Class that simulates a player making the move which maximises the number of moves,
    defined as the number of times a player distributes marbles from a chosen bucket (one execution of Board.make_player_move()).
    """
    ...
    def score(self, board):
        n_moves = board.n_moves_player_one if self.player == 1 else board.n_moves_player_two
        return n_moves

    def get_move_with_max_moves(self, score_move_holder):
    ...
    def move(self, board):
        """Choose the move from board to maximise the player's n_moves.
            If there are multiple possible moves, then the choice is random"""

        if board.player != self.player:
            raise ValueError(f'board player is {board.player} and MaxMovesPlayer is {self.player}. Please check your logic')
        ...
        return self.get_move_with_max_moves(score_move_holder)
    ...
    def get_move_score(self, move, board, first_move=True):
        ...
        while True:
            ...
            if board_copy.side == None and board_copy.position == 0:
                ...
                choice = self.get_move_with_max_moves(moves_scores)
